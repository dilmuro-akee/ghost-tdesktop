...
void Session::checkTTLs() {
	...
	if (!expired.empty()) {
		// GHOST MODE (toggleable): keep view-once / disappearing media instead
		// of letting it auto-destruct, so it can be viewed and saved.
...
void Session::processMessagesDeleted(
	...
	if (!toDestroy.empty()) {
		// GHOST MODE (toggleable anti-delete): keep messages that others (or
		// your other devices) delete. Your own deletions use a separate path
		// (Histories::deleteMessages) and still work regardless.
		if (!Ghost::AntiDelete.value()) {
...
void Session::processNonChannelMessagesDeleted(const QVector<MTPint> &data) {
	...
	if (!toDestroy.empty()) {
		// GHOST MODE (toggleable anti-delete): keep remotely-deleted messages.
		if (!Ghost::AntiDelete.value()) {
			notifyItemsAboutToBeDestroyed(toDestroy);
			for (const auto &item : toDestroy) {
				item->destroy();
