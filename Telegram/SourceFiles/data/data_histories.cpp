...
void Histories::sendReadRequest(not_null<History*> history, State &state) {
	...
	sendRequest(history, RequestType::ReadInbox, [=](Fn<void()> finish) {
		...
		// GHOST MODE (toggleable): don't send read receipts to the server
		// (messages.readHistory / channels.readHistory) so the other side never
		// sees the blue "read" ticks. We still finalize the local read-state
		// bookkeeping, deferred so sendRequest() can register this request
		// before finished() tries to clean it up.
		if (Ghost::NoReadReceipts.value()) {
