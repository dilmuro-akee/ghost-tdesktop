...
MediaCheckResult CheckMessageMedia(const MTPMessageMedia &media) {
	...
	return media.match([](const MTPDmessageMediaEmpty &) {
		...
		// GHOST MODE: treat self-destruct (ttl) photos as normal media so they
		// render with the normal opener instead of "view it on your mobile".
		if (data.vttl_seconds() && !Ghost::SaveViewOnce.value()) {
		...
		// GHOST MODE: treat view-once / self-destruct videos, round notes and
		// voice messages as normal media (render if the file is present).
		if (data.vttl_seconds() && !Ghost::SaveViewOnce.value()) {
			if (data.is_video()) {
				return Result::HasUnsupportedTimeToLive;
			} else if (!document) {
