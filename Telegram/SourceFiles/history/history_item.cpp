...
// GHOST MODE: auto-save view-once / self-destruct documents (video, round
// notes, voice) to a folder the moment they arrive — while the file reference
// is still valid — so they're never lost. Only runs for fresh messages.
void GhostAutoSaveOneTime(
		not_null<HistoryItem*> item,
		const MTPDmessage &message) {
	if (!Ghost::AutoSaveOneTime.value()) {
		return;
	}
	...
	if (base::unixtime::now() - message.vdate().v > 600) {
		return; // old message — its view-once reference is already dead.
	}
	media->match([&](const MTPDmessageMediaDocument &data) {
		const auto doc = data.vdocument();
		if (!data.vttl_seconds() || !doc) {
			return;
		}
		const auto document = item->history()->owner().processDocument(*doc);
		auto base = Core::App().settings().downloadPath();
		if (base.isEmpty()) {
			base = QStandardPaths::writableLocation(
				QStandardPaths::DownloadLocation);
		}
		const auto folder = base + u"/TelegramGhost/"_q;
		QDir().mkpath(folder);
		auto name = document->filename();
		if (name.isEmpty()) {
			name = u"onetime"_q;
		}
		const auto path = folder + QString::number(item->id.bare) + '_' + name;
...
std::unique_ptr<Data::Media> HistoryItem::CreateMedia(
	...
	return media.match([&](const MTPDmessageMediaContact &media) -> Result {
		...
		// GHOST MODE: allow building self-destruct (ttl) photos so they render
		// on desktop instead of the "view it on your mobile" placeholder.
		if (media.vttl_seconds() && !Ghost::SaveViewOnce.value()) {
			LOG(("App Error: "
		...
		// GHOST MODE: allow building self-destruct (ttl) videos so they render
		// on desktop instead of the "view it on your mobile" placeholder.
		if (media.vttl_seconds() && media.is_video()
				&& !Ghost::SaveViewOnce.value()) {
			LOG(("App Error: "
		...
		return document->match([&](const MTPDdocument &document) -> Result {
			...
			return std::make_unique<Data::MediaFile>(item, data, Args{
				// GHOST MODE: drop the self-destruct timer so view-once videos,
				// round notes and voice play with the normal opener and stay.
				.ttlSeconds = (Ghost::SaveViewOnce.value()
					? TimeId(0)
...
: HistoryItem(history, {
	...
	if (checked == MediaCheckResult::Unsupported) {
		...
		// GHOST MODE: when "Save one-time media" is on, fall through to the
		// normal media path below so self-destruct photos/videos actually
		// render (and stay) instead of the "view it on your mobile" message.
		...
		if (const auto media = data.vmedia()) {
			setMedia(*media);
			// GHOST MODE: remember view-once / self-destruct media so Forward
			// re-uploads it instead of failing with MESSAGE_ID_INVALID.
			if (Ghost::SaveViewOnce.value()) {
				const auto hasTtl = media->match(
					[](const MTPDmessageMediaPhoto &d) {
						return d.vttl_seconds() ? true : false;
					}, [](const MTPDmessageMediaDocument &d) {
						return d.vttl_seconds() ? true : false;
					}, [](const auto &) { return false; });
				if (hasTtl) {
					Ghost::RememberViewOnce(fullId());
				}
			}
		}
	...
	GhostAutoSaveOneTime(this, data);
}
...
void HistoryItem::applyEdition(HistoryMessageEdition &&edition) {
	...
	if (updatingSavedLocalEdit) {
		...
		// GHOST MODE (toggleable): keep the original text visible when a message
		// is edited, appended after the new text so you can see what changed.
		if (Ghost::ShowEditOriginal.value()
			&& !_text.empty()
			&& _text.text != updatedText.text) {
			const auto marker = QString::fromUtf8("\n\n— original: ");
			auto previous = _text.text;
			const auto cut = previous.indexOf(marker);
			if (cut >= 0) {
				previous = previous.mid(0, cut);
			}
			if (!previous.isEmpty() && !updatedText.text.contains(marker)) {
				updatedText.text += marker + previous;
			}
		}
...
bool HistoryItem::forbidsSaving() const {
	// GHOST MODE (toggleable): ignore the "restrict saving content" (NoForwards)
	// protection so media can be downloaded from protected channels/groups — the
	// data is already on the client to be displayed. (Paid/extended media still
	// can't be saved because the actual file isn't present.)
	if (!Ghost::DownloadRestricted.value() && forbidsForward()) {
		return true;
	}
	if (const auto invoice = _media ? _media->invoice() : nullptr) {
...
crl::time HistoryItem::getSelfDestructIn(crl::time now) {
	// GHOST MODE (toggleable): never let view-once / self-destruct media expire,
	// so you can keep viewing and saving it.
	if (Ghost::SaveViewOnce.value()) {
		return 0;
	}
	if (const auto selfdestruct = Get<HistoryServiceSelfDestruct>()) {
