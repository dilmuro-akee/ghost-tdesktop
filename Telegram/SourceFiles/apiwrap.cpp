...
void ApiWrap::forwardMessages(
	...
	for (auto i = begin(draft.items); i != end(draft.items);) {
		const auto item = *i;
		const auto media = item->media();
		// GHOST MODE: view-once / self-destruct media can't be forwarded by id
		// (server returns MESSAGE_ID_INVALID), so re-upload the file instead and
		// add a "Forwarded from <name>" caption to keep attribution.
		const auto viewOnce = Ghost::IsViewOnce(item->fullId())
			&& media
		...
		if (item->isSavedMusicItem()) {
			...
			message.textWithTags.text = QString::fromUtf8("Forwarded from ")
				+ item->from()->name();
