...
void SendProgressManager::send(const Key &key, int progress) {
	// GHOST MODE (toggleable): never broadcast typing / recording / uploading
	// indicators, so the other side never sees "typing…", "sending photo…".
	if (Ghost::StealthTyping.value()) {
		return;
	}
	if (skipRequest(key)) {
		return;
