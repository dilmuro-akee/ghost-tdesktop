...
bool OverlayWidget::hasCopyMediaRestriction(bool skipPremiumCheck) const {
	...
	// GHOST MODE (toggleable): allow downloading media from content-protected
	// chats directly in the media viewer (Save button stays enabled).
	if (Ghost::DownloadRestricted.value()) {
		return false;
	}
...
void OverlayWidget::close() {
	...
	// GHOST/UX: if a video is playing, keep it alive in Picture-in-Picture
	// instead of stopping it when the viewer closes (opening a chat, Escape,
	// clicking away). _showAsPip guards against recursion (switchToPip() calls
	// close() at its end).
	if (Ghost::VideoPip.value()
		&& !_showAsPip
		&& _streamed
		&& _streamed->controls
		&& _document) {
