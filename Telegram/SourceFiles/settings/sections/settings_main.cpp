...
void BuildSectionButtons(SectionBuilder &builder) {
	...
	// Ghost Mode (this fork): a dedicated entry opening a box of ghost toggles.
	builder.addButton({
		...
		.onClick = [=] {
			controller->uiShow()->show(Box([=](
				...
				box->setTitle(rpl::single(u"Ghost Mode"_q));
				const auto container = box->verticalLayout();
				const auto addRow = [&](
						const QString &text,
						base::options::toggle *opt) {
					const auto button = AddButtonWithIcon(
						container,
						rpl::single(text),
						st::settingsButton);
					button->toggleOn(rpl::single(opt->value()));
					button->toggledChanges(
					) | rpl::on_next([opt](bool value) {
						opt->set(value);
					}, button->lifetime());
				};
				addRow(u"No read receipts"_q, &Ghost::NoReadReceipts);
				addRow(u"Hide online status"_q, &Ghost::HideOnline);
				addRow(u"Stealth typing"_q, &Ghost::StealthTyping);
				addRow(u"Anti-delete"_q, &Ghost::AntiDelete);
				addRow(
					u"Download restricted content"_q,
					&Ghost::DownloadRestricted);
				addRow(u"Auto-save one-time media"_q, &Ghost::AutoSaveOneTime);
				addRow(u"Show edited originals"_q, &Ghost::ShowEditOriginal);
				addRow(u"Keep video playing (PiP)"_q, &Ghost::VideoPip);

				// Download all custom + premium emoji you have, to local cache.
				const auto dl = AddButtonWithIcon(
					container,
					rpl::single(u"⬇ Download all emoji (incl. premium)"_q),
					st::settingsButton);
				dl->setClickedCallback([=] {
					// Save real emoji files into a folder (not just the cache).
					auto base = Core::App().settings().downloadPath();
					if (base.isEmpty()) {
						base = QStandardPaths::writableLocation(
							QStandardPaths::DownloadLocation);
					}
					const auto folder = base + u"/TelegramGhost/Emoji/"_q;
					QDir().mkpath(folder);
					auto &stickers = controller->session().data().stickers();
					auto count = 0;
					for (const auto setId : stickers.emojiSetsOrder()) {
						const auto i = stickers.sets().find(setId);
						if (i == stickers.sets().end()) {
							continue;
						}
					...
					controller->showToast(count
						? (u"Saving "_q
							+ QString::number(count)
							+ u" emoji to: "_q
							+ folder)
						: u"No installed emoji packs found to save."_q);
				});

				box->addButton(tr::lng_box_ok(), [=] { box->closeBox(); });
			}));
		},
		.keywords = { u"ghost"_q, u"stealth"_q, u"privacy"_q },
	});
